...
- Operations:
  ...
  - `addImport(file, specifier, source)` — add an import, merging into an existing
    ...
    `changes: 0`. A newly inserted statement consumes exactly one newline after the
    last import, so the blank line separating the import block from the code below
    it survives (#103).
