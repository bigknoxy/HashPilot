...
| `locking.ts` | Advisory file locks (`.hashpilot/locks/`, PID + heartbeat, stale reclaim after 30s). `acquireLock` for one file; `acquireSortedLocks` for a set, sorted to prevent deadlock on overlapping file sets. Locks are **not** re-entrant — concurrent writers in one process must still exclude each other — so `batch-edit` passes `alreadyLocked` to the router instead of nesting an acquire. Release functions are idempotent. |
